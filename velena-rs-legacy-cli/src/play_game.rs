...
use velena_rs::ai::compute_ai_move;
...
fn parse_input_string(input_string: &str) -> Result<Board, CLIError> {
...
pub fn play_game(input_string: &str) -> Result<usize, CLIError> {
    ...
    Ok(compute_ai_move(parse_input_string(input_string)?) + 1) //Answer in the range 1..=7
}